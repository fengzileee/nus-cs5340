...
class SamplingVisualizer:
    ...
    def __init__(
        self, N_future: int = 4, N_history: int = None, sample_size: int = 100
    ):
        self._N = N_future
    ...
    def latest_samples(self):
        """The latest samples from the last visualization."""
    ...
    def visualize_to_axis(self, axis, predictor: TrajPredictor, trajectory: np.ndarray):
        ...
        samples = predictor.sample(trajectory[0:N_his, :], count=self._sample_size)
...
def visualize_covariance_to_axis(axis, samples):
    """Visualize the covariance ellipse.

    Args:
        axis: matplotlib axis object.
        samples: Samples drawn by a predictor.
    """

    def cep(x, y, axis, color, alpha_scale=1.0):
    ...
    cep([s[0, 0] for s in samples], [s[0, 1] for s in samples], axis, "red")
